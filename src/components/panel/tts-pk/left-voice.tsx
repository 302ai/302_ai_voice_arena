...
const LeftVoice = () => {
  ...
  useEffect(() => {
    if (!form.leftVoice) {
      setForm((prev) => ({ ...prev, leftVoice: "random" }));
    } else if (form.leftVoice !== "random") {
      // 从已设置的声音中提取平台和语言
      const parts = form.leftVoice.split(":");
      if (parts.length > 0) {
  ...
  const handlePlatformChange = (value: string) => {
    ...
    if (value === "random") {
      setForm({ ...form, leftVoice: "random" }); // 设置为随机模式
    } else {
      setForm({ ...form, leftVoice: "" }); // Reset voice when platform changes
    }
  };
  ...
  const handleLocaleChange = (value: string) => {
    ...
    setForm({ ...form, leftVoice: "" }); // Reset voice when locale changes
  };

  // Handle voice selection
  const handleVoiceChange = (value: string) => {
    setForm({ ...form, leftVoice: `${value}` });
  };
  ...
  const voiceSelectOptions = useMemo(() => {
    return availableVoices.map((voice) => {
      ...
      const isDisabled = form.rightVoice === voiceId; // 检查是否已被右边选中
